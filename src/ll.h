...
public:
    ...
    explicit LinkedList(
        const std::vector<ElementType> &values);  // Initialize from vector

    // Big 5
    LinkedList(const LinkedList &source);      // Copy constructor
    LinkedList(LinkedList &&source) noexcept;  // Move constructor
    ~LinkedList();                             // Destructor
    LinkedList<ElementType> &operator=(
        const LinkedList<ElementType> &source);  // Copy assignment operator
    LinkedList<ElementType> &operator=(
        LinkedList<ElementType> &&source) noexcept;  // Move assignment operator
    ...
    class const_iterator
        : std::iterator<std::forward_iterator_tag, const ElementType> {
