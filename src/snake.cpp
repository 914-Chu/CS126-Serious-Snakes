...
SnakeBody* Snake::getHead() const { return head_; };

ofVec2f Snake::getBodySize() const { return body_size_; };
...
void Snake::update() {
    ...
    switch (current_direction_) {
        case UP:
            head_->position.set(head_->position.x,
                                head_->position.y - body_size_.y);
            break;
        case DOWN:
            head_->position.set(head_->position.x,
                                head_->position.y + body_size_.y);
            break;
        case LEFT:
            head_->position.set(head_->position.x - body_size_.x,
                                head_->position.y);
            break;
        case RIGHT:
            head_->position.set(head_->position.x + body_size_.x,
...
bool Snake::isDead() const {
    ...
    if (head_->position.x < 0 || head_->position.y < 0 ||
    ...
    ofRectangle head_rect(head_->position.x, head_->position.y, body_size_.x,
                          body_size_.y);
    for (SnakeBody* curr = head_->next; curr; curr = curr->next) {
        ofRectangle body_rect(curr->position.x, curr->position.y, body_size_.x,
                              body_size_.y);
...
void Snake::eatFood(ofColor newBodyColor) {
    ...
    // The current position of the new tail is one unit in the opposite
    // direction of the snakes current movement
    switch (current_direction_) {
        case UP:
            new_body->position.set(last_body->position.x,
                                   last_body->position.y + body_size_.y);
            break;
        case DOWN:
            new_body->position.set(last_body->position.x,
                                   last_body->position.y - body_size_.y);
            break;
        case LEFT:
            new_body->position.set(last_body->position.x + body_size_.x,
                                   last_body->position.y);
            break;
        case RIGHT:
            new_body->position.set(last_body->position.x - body_size_.x,
                                   last_body->position.y);
...
int Snake::getFoodEaten() const { return snake_body.size(); }

SnakeDirection Snake::getDirection() const { return current_direction_; }
